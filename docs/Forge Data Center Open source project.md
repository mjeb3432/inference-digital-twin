...
## Additional Artifacts and Tools Added to Notion (Non-Code)

The following companion artifacts are included to make the project operationally useful beyond the simulation UI.

### Artifact 1: Assumptions Register v1

Purpose: one place to track every major assumption, its owner, evidence, confidence, and next review date.

Template columns:

- Assumption ID
- Domain (Power, Cooling, Network, Runtime, Orchestration, Economics)
- Assumption statement
- Source type (External/Internal/Inference)
- Source link or file anchor
- Confidence (Low/Med/High)
- Last validated date
- Owner
- Decision impact if wrong

### Artifact 2: Scenario Intake Worksheet

Purpose: standardize scenario creation before simulation.

Sections:

- Business objective (latency-first, cost-first, capacity-first)
- Workload profile
- Hard constraints (budget, power cap, launch date)
- Must-have reliability posture
- Planned comparison scenarios (A/B/C)

### Artifact 3: Sensitivity Sweep Planner

Purpose: identify which inputs move outcomes the most.
...
- Sweep range
- Step size
- Primary metrics watched (TTFT/TPS/TCO/Power/Carbon)
- Observed sensitivity
- Recommendation

### Artifact 4: Decision Memo Template

Purpose: convert simulation output into an executive decision record.
...
- Final choice and trigger to revisit

### Artifact 5: Validation & QA Checklist

Purpose: ensure each published scenario/report is reproducible and policy-safe.

Checklist:

- Scenario contract valid
- Artifact compatibility pass
- No phase validation errors
- Report includes provenance fields
- Limitations reviewed
- Decision memo attached

### Artifact 6: Risk Register (Model + Program)

Purpose: track model risk, data drift risk, and execution risk separately.

Columns:

- Risk ID
- Risk type (Model/Data/Operational)
- Trigger condition
- Detection signal
- Severity
- Mitigation action
...
### Artifact 7: Experiment Backlog

Purpose: queue future studies without losing rationale.
